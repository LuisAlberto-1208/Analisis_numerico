...
int main()
{
    ...
    float MatrizPuntos[puntos][2];

    for(int i=0; i<puntos; i++)
    {
        for(int j=0; j<2; j++)
        {
            scanf("%f", &MatrizPuntos[i][j]);
        }
    }

    float A[puntos][puntos]; //Matriz A (Coeficientes de las ecuaciones del sistema)
    ...
    float b[puntos]; //Vector solucion
    ...
    float Id[puntos][puntos]; //Matriz identidad
    ...
    //Coeficientes
    printf("Los coeficientes de la funcion, de menor a mayor exponente son los siguientes\n");
    for(int i=0; i<puntos; i++)
    {
        printf("C %d = %f\n", i, aux[i]);
    }
